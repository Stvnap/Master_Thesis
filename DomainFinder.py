...
BATCH_SIZE = 20 if VRAM >= 24 else 15 if VRAM >= 16 else 10 if VRAM >= 8 else 5
LR = 1e-5
WEIGHT_DECAY = 1e-2
EPOCHS = 1000
STUDY_N_TRIALS = 0  # number of optuna trials
...
def main_trainer(Final_training=False):
    ...
    if RANK == 0:
        study = optuna.create_study(
            direction="minimize",
            storage=f"sqlite:////scratch/tmp/sapelt/Master_Thesis/logs/{PROJECT_NAME}/optuna_study.db",
            load_if_exists=True,
...
if __name__ == "__main__":
    if TRAINER_MODE is True:
        main_trainer(Final_training=True)
