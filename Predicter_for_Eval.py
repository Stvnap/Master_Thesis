...
class Predicter_pipeline:
    ...
    def __init__(self, model_path, df_path, flank_size, step, batch_size=32):
        ...
        predictions_new, true_labels_new, self.predictions_bool_new = self._predicting(
            model_path,
            self.seqarray_final,
        )
        ...
        overlaps, ids, predictions, window_positions, boundaries = self._binary_finder(
            self.seqarray_final
        )
    ...
    def _multiplier(self, seqarray_full, sliding, window_size, flank_size, step):
        ...
        for idx, nested_list in enumerate(sliding):
            ...
            current_id = row["ID"]
            current_boundary = row["Boundaries"]
            start_pos = row["Start"]
            end_pos = row["End"]
    ...
    def _binary_finder(self, seqarray_final):
        ...
        return overlaps, ids, predictions, window_positions, boundaries
    ...
    def _hitcomparer(self, ids, predictions, overlaps, threshold=0.5):
        ...
        print(f"Baseline accuracy:         {accuracy(baseline_flags):.5f}")
        print(f"Threshold+Cons accuracy:   {accuracy(thr_flags):.5f}")
        print(f"Baseline precision:        {precision(baseline_flags):.5f}")
        print(f"Threshold+Cons precision:  {precision(thr_flags):.5f}")
