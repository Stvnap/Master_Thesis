...
CSV_PATH = "/global/students/research/sapelt/Masters/MasterThesis/Dataframes/Evalsets/DataEvalSwissProt2d_esm_150w_shuffled.csv"
CATEGORY_COL = "categories"
SEQUENCE_COL = "Sequences"
MODEL_PATH = "/global/students/research/sapelt/Masters/MasterThesis/models/FINAL/t33_ALL_2d.pt"
CACHE_PATH = "/global/students/research/sapelt/Masters/MasterThesis/pickle/Predicer_embeddings_test_1000_wsize150_shuffled.pkl"
...
class ESMDataset:
    ...
    def esm_loader(self):
        ...
        # First move to cuda:0 explicitly
        model = model.to('cuda:0')
        
        # Then wrap with DataParallel (if multiple GPUs available)
        if torch.cuda.device_count() > 1:
            model = torch.nn.DataParallel(model, device_ids=[0, 1, 2])  # Explicitly specify device IDs
    ...
    def _embed(self, seqs):
        ...
        for batch_idx, (batch_labels, batch_strs, batch_tokens) in enumerate(loader):
            try:
                ...
                if "out of memory" in str(e):
                    ...
                    for i, (label, seq_str) in enumerate(zip(batch_labels, batch_strs)):
                        try:
                            ...
                            dummy = torch.zeros(1, 1280)  # ESM-2 650M embedding dimension
                            all_vecs.append(dummy)
                else:
                    # Re-raise the error instead of silently passing
                ...
                # Add fallback embeddings for this batch
                batch_size = len(batch_labels)
                dummy = torch.zeros(batch_size, 1280)
                all_vecs.append(dummy)

        # Check if we have any embeddings before concatenating
        if not all_vecs:
            raise RuntimeError("No embeddings were generated - all batches failed!")
...
def main():
        ...
        while i < len(endlist):
            ...
            while j < len(endlist) and endlist[j]["ID"] == current_row["ID"]:
                ...
                if isinstance(current_row["Domain_Start"], list):
                    current_row["Domain_Start"].append(endlist[j]["Domain_Start"])
                else:
                ...
                if isinstance(current_row["Domain_End"], list):
                    current_row["Domain_End"].append(endlist[j]["Domain_End"])
                else:
    ...
    endlist.to_csv("/global/students/research/sapelt/Masters/MasterThesis/Results/Predicter_from_ESM_final_result.csv", index=False)
...
def plotter(
    ...
    if Name == "Start":
        colors = cm.get_cmap("tab10").colors[0]
    else:
        colors = cm.get_cmap("tab10").colors[1]


    plt.figure(figsize=(8, 5))
    sns.histplot(errors, bins=bins, color=colors, kde=False, stat='probability',edgecolor='black')
    plt.xticks(bins[::2], fontsize=11)
    plt.yticks(fontsize=11)
    # plt.xlim(-5,105)
    plt.title(f"Normalized Distribution of {Name} Boundary Absolute Errors")
    plt.xlabel("Absolute error in residues")
    plt.ylabel("Fraction")
    # plt.grid(axis="y", alpha=0.5, linestyle="--")
    plt.tight_layout()
    plt.savefig(
        f"/global/students/research/sapelt/Masters/MasterThesis/Evalresults/ESM/histogram_{Name}_errors.png", dpi=600, bbox_inches="tight"
    )
    plt.show()
...
def boxplotter(errors, classes, Name):
    ...
    violin4 = ax.violinplot(data, showmeans=True)

    colors = cm.get_cmap("tab10").colors
    for i, pc in enumerate(violin4['bodies']):
        pc.set_facecolor(colors[0] if Name == "Start" else colors[1])
        pc.set_alpha(0.7)
    ...
    ax.set_ylabel("Absolute error in residues")
    # ax.grid(axis="y", linestyle="--", alpha=0.5)
    ax.set_ylim(bottom=-5)

    plt.tight_layout()
    plt.savefig(f"/global/students/research/sapelt/Masters/MasterThesis/Evalresults/ESM/boxplot_{Name}.png", dpi=600, bbox_inches="tight")
    plt.show()
...
if __name__ == "__main__":
    ...
    # Cache the results for future plotting without re-running predictions
    results_cache_path = "/global/students/research/sapelt/Masters/MasterThesis/pickle/prediction_results_cache.pkl"

    # Try to load cached results first
    try:
        with open(results_cache_path, 'rb') as f:
            cached_results = pickle.load(f)
            all_start_residues = cached_results['all_start_residues']
            errors_start = cached_results['errors_start']
            all_end_residue = cached_results['all_end_residue']
            errors_end = cached_results['errors_end']
            true_class = cached_results['true_class']
        print("Loaded cached prediction results from disk.")
    except FileNotFoundError:
        # Cache doesn't exist, save current results
        results_to_cache = {
            'all_start_residues': all_start_residues,
            'errors_start': errors_start,
            'all_end_residue': all_end_residue,
            'errors_end': errors_end,
            'true_class': true_class
        }
        with open(results_cache_path, 'wb') as f:
            pickle.dump(results_to_cache, f)
        print("Saved prediction results to cache.")
